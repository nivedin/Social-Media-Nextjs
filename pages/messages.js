...
function Messages({ chatsData, user, errorLoading }) {
  ...
  useEffect(() => {
    const loadMessages = () => {
      ...
      socket.current.on("messagesLoaded", ({ chat }) => {
        ...
        divRef.current && scrollChatToBtm(divRef);
      });

      socket.current.on("noChatFound", async () => {
        const { name, profilePicUrl } = await getUserInfo(router.query.message);
        console.log("nodata", name, profilePicUrl);
  ...
  useEffect(() => {
    if (socket.current) {
      socket.current.on("messageSend", ({ newMsg }) => {
  ...
  useEffect(() => {
    messages.length > 0 && scrollChatToBtm(divRef);
  }, [messages]);
  ...
  const deleteChat = async (messagesWith) => {
    try {
      ...
      console.error("Error deleting the chat");
    }
  };
